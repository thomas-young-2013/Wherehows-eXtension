...
public class CASUtils {
    ...
    public static String getCasLoginUrl(Boolean possibleGateway) {
        String casLoginUrl = Play.application().configuration().getString("cas.loginUrl");
        // we add the service URL (the reverse route for SecureCas.
        casLoginUrl += "?service=" + Cache.get("url_" + session().get("uuid"));
    ...
    public static String getCasLogoutUrl() {
        if (isCasMockServer()) {
            ...
            return Play.application().configuration().getString("cas.logoutUrl");
        }
    }
    ...
    private static String getCasServiceValidateUrl() {
        ...
        if (isCasMockServer()) {
            ...
            casServiceValidateUrl = Play.application().configuration().getString("cas.validateUrl");
        }
    ...
    private static String getCasProxyCallBackUrl() {
        ...
        if (!casProxyCallBackUrl.contains("https") && !isCasMockServer()) {
            ...
            if (Play.application().configuration().getString("application.baseUrl.ssl") != null
                    && !Play.application().configuration().getString("application.baseUrl.ssl").equals("")) {
                casProxyCallBackUrl = Play.application().configuration().getString("application.baseUrl.ssl");
            }
            else {
    ...
    private static String getCasProxyUrl() {
        ...
        if (isCasMockServer()) {
            ...
            casProxyUrl = Play.application().configuration().getString("cas.proxyUrl");
        }
        return casProxyUrl;
    }
    ...
    private static Boolean isProxyCas() {
        ...
        if (Play.application().configuration().getString("cas.proxyUrl") != null
                && !Play.application().configuration().getString("cas.proxyUrl").equals("")) {
    ...
    public static Boolean isCasMockServer() {
        ...
        if (Play.application().configuration().getString("cas.mockserver") != null
                && Play.application().configuration().getString("cas.mockserver").equals("true") && Play.isDev() == true) {
    ...
    public static CASUser valideCasTicket(String ticket) throws IOException, SAXException,
        ...
        if (list.getLength() > 0) {
            ...
            if (isProxyCas()) {
                ...
                Cache.set("pgt_" + user.getUsername(), pgt);
            }
        }
    ...
    public static String getProxyTicket(String username, String serviceName) throws IOException, SAXException,
        ...
        Document response = WS.url(url).get().get(10).asXml();
